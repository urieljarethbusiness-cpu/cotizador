...
async def _insert_servicio_cotizado(conn, cotizacion_id: str, svc, bucefalo_servicio_id, now, es_doble: bool = False) -> None:
    ...
    precio = 0 if modelo_cobro == "demanda" else svc.precio
    ...
    await conn.execute(
        ...
        INSERT INTO "ServicioCotizado"
            (id, "cotizacionId", "servicioCatalogoId", nombre, "esPersonalizado",
             ...
             opcion, fase, "tipoPago", precio, "tiempoEntrega", entregables, beneficios, notas,
...
async def _find_or_create_cliente(conn, nombre: str, empresa: str, email: str, telefono: str, rfc: str = "") -> str:
    row = await conn.fetchrow(
    ...
    if row:
        if email or telefono or rfc:
            await conn.execute(
                'UPDATE "Cliente" SET email = COALESCE($1, email), telefono = COALESCE($2, telefono), rfc = COALESCE($3, rfc), "updatedAt" = NOW() WHERE id = $4',
                email or None,
                telefono or None,
                rfc or None,
                row["id"],
    ...
    new_id = _cuid()
    await conn.execute(
        'INSERT INTO "Cliente" (id, nombre, empresa, email, telefono, rfc, "createdAt", "updatedAt") VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())',
        new_id,
        nombre,
        empresa or None,
        email or None,
            ...
            await conn.execute(
                ...
                INSERT INTO "Cotizacion"
                    (id, numero, fecha, vigencia, moneda, "tipoCambio", proyecto, "esquemaPago",
                     estado, "incluirBonos", "incluirFinanciamiento", "incluirIva", "esDoble", "opcionesMetadata",
                     observaciones, "clienteId", "asesorId", "createdAt", "updatedAt")
                VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'borrador',$9,$10,$11,$12,$13,$14,$15,$16,$17,$17)
                """,
                cot_id,
                body.numero,
                body.fecha,
...
async def update_cotizacion(
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            ...
            _add("incluirIva", '"incluirIva"', body.incluirIva)
